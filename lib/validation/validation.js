...
function validateProducts(items, meta) {
    ...
    var result = [];
    var realAvgPrice;
    // Get average Price for varianz calculation
    _getAvg(items, function (avgPrice) {
        // build varianz to get outliers
    ...
    // calculate real Average value only with items which passed both checks
    realAvgPrice = realAvg.getRealAvg(result, meta);
    // Check if no data passed the checks
    if(isNaN(realAvgPrice)) {
        realAvgPrice = 0;
    }
...
// build the result which will be send to the frontend
...
// check how many validators were true to get indicators for traffic-light system in frontend
